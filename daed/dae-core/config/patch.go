...
// patchEchTunnel validates the optional ech_tunnel section: the tunnel
// server is mandatory whenever the section is present.
func patchEchTunnel(params *Config) error {
	if params.EchTunnel == nil {
		return nil
	}
	if params.EchTunnel.Server == "" {
		return fmt.Errorf("ech_tunnel: \"server\" is required when the ech_tunnel section is present")
	}
	return nil
}
