...
// EchTunnel is the optional "ech_tunnel" section: it runs the ech-workers
// ECH websocket tunnel client (see component/echworkers) inside dae,
// exposing the tunnel as a local SOCKS5/HTTP proxy. The tunnel can also be
// used natively as a node via "echws://" links in the "node" section.
type EchTunnel struct {
	// Listen enables the local SOCKS5/HTTP proxy server when non-empty,
	// e.g. '127.0.0.1:1080'.
	Listen string `mapstructure:"listen"`
	// Server is the wss tunnel server in 'host:port[/path]' form.
	Server string `mapstructure:"server"`
	// ServerIP optionally pins the TCP connection to a fixed IP.
	ServerIP string `mapstructure:"ip"`
	// Token is sent as the websocket subprotocol for authentication.
	Token string `mapstructure:"token"`
	// DNSServer is the DoH server used to fetch the ECH config.
	// Defaults to 'dns.alidns.com/dns-query'.
	DNSServer string `mapstructure:"dns"`
	// EchDomain is the domain queried for its HTTPS (ECH) record.
	// Defaults to 'cloudflare-ech.com'.
	EchDomain string `mapstructure:"ech_domain"`
}
...
type Config struct {
	...
	// EchTunnel is nil when the section is absent.
	EchTunnel *EchTunnel `mapstructure:"ech_tunnel" desc:"EchTunnelDesc"`
}
